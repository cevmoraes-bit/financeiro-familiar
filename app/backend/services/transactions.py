...
class TransactionsService:
    ...
    async def get_list(
        ...
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Transactions, field):
                        column = getattr(Transactions, field)
            ...
            stmt = select(Transactions, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Transactions, field_name):
                        stmt = stmt.order_by(getattr(Transactions, field_name).desc())
                else:
                    if hasattr(Transactions, sort):
                        stmt = stmt.order_by(getattr(Transactions, sort))
            else:
                stmt = stmt.order_by(Transactions.id.desc())
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Transactions]:
        ...
        try:
            ...
            result = await self.db.execute(
                select(Transactions).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
